 ...
 * @date 2021/6/6 18:03
 */
public class WxUserDataDecrypt {

	private String appId;

	private String sessionKey;

	public WxUserDataDecrypt(String appId, String sessionKey) {
		this.appId = appId;
	 ...
	 * @throws Exception
	 */
	public WxUserInfoDTO decrypt(String encryptedData, String iv) throws Exception {
		String jsonStr;
		try {
		...
		WxUserInfoDTO userInfoDTO = FormatUtil.deserializeCamelCaseJson2Object(jsonStr, WxUserInfoDTO.class);
		if (ObjectUtils.isEmpty(userInfoDTO)
			|| ObjectUtils.isEmpty(userInfoDTO.getWatermark())) {
			throw new Exception("decode error");
		}
		if (!userInfoDTO.getWatermark().getAppid().equals(this.appId)) {
			throw new Exception("decode error");
		}
