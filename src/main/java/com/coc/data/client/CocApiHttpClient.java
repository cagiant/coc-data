...
public class CocApiHttpClient implements HttpClient {
    ...
    public CocApiHttpClient(WebClient.Builder webClientBuilder) {
        this.webClient = webClientBuilder
            .filter((req, next) -> {
                ...
                request.headers( httpHeaders -> {httpHeaders.setBearerAuth(cocToken);});
    ...
    public PlayerDTO getPlayerInfoByTag(String playerTag) {
        Flux<PlayerDTO> playerInfoDTOFlux = this.webClient.get().uri(UrlConstants.PLAYER_INFO, playerTag)
            .retrieve().bodyToFlux(PlayerDTO.class);

        return playerInfoDTOFlux.singleOrEmpty().block();
    }
}
